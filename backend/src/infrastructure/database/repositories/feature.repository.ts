...
export class FeatureRepository implements IFeatureRepository {
  ...
  async findByTitle(title: string): Promise<Feature | null> {
    const doc = await FeatureModel.findOne({
      title,
      deletedAt: null,
    });
  ...
  async update(id: string, data: Partial<CreateFeatureInput>): Promise<Feature | null> {
    const doc = await FeatureModel.findOneAndUpdate(
  ...
  async list(filters?: FeatureListFilters): Promise<FeatureListResult> {
